...
import BeeswarmForceApplied from "$components/charts/Beeswarm/BeeswarmForce.applied.svelte";

import { autoType } from "d3";
import { preprocessor, merge } from "$utils/data";

import countries from "$data/wid_countries.csv";
import gini from "$data/data_all.csv";
...
export default {
  components: [BeeswarmForceApplied],
  steps: [
    {
      layers: [
        {
...

    {
      ...
      description: {
        title: `1990s (${
          ...
          data.filter((d) => d.decade === 1990).length
        })`,
